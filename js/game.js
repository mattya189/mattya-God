...
class Battle {
  ...
  place(side, c, slot) {
    ...
    if (u.ultimate && this.alive(u) && ULTIMATES[u.element]) {
      const ul = ULTIMATES[u.element];
      this.emit({ t: 'ability', side, uid: u.uid, skill: ul.name, ultimate: true, text: `${this.unitLabel(u)}の「${ul.name}」：${effectText(ul.effect)}` });
      this.applyEffect(u, ul.effect, {});
      this.resolveDeaths();
    }
    this.checkBonds(u);
    this.resolveDeaths();
...
function openCardDetail(id) {
  ...
  const body = openDialog(`
    <div class="bigcard" style="--el:${elColor(c.element)}">
      ...
      <div class="bc-sec"><h3>奥義（★5で使える）</h3>「${esc(ult.name)}」登場時に1回：${esc(effectText(ult.effect))}</div>
...
function openUnitInfo(view, uid) {
  ...
  panel.innerHTML = `<div class="ui-card" style="--el:${elColor(u.element)}">
    ...
    ${u.ultimate ? `<div class="bc-sec"><b>「${esc(ULTIMATES[u.element].name)}」</b>（登場時に発動）</div>` : ''}
